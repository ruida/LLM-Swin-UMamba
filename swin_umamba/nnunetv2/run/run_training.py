...
def run_ddp(rank, dataset_name_or_id, configuration, fold, tr, p, use_compressed, disable_checkpointing, c, val,
    ...
    nnunet_trainer.perform_actual_validation(npz, report_text=report_text)
    cleanup_ddp()
...
def run_training(dataset_name_or_id: Union[str, int],
                 ...
                 device: torch.device = torch.device('cuda'), report_text=None):
    ...
    if num_gpus > 1:
        ...
        if val_with_best:
            nnunet_trainer.load_checkpoint(join(nnunet_trainer.output_folder, 'checkpoint_best.pth'))
        nnunet_trainer.perform_actual_validation(export_validation_probabilities, report_text=report_text)
