...
class nnUNetTrainer(object):
    ...
    def plot_network_architecture(self):
        ...
        if self.local_rank == 0:
            try:
                ...
                # broken.
                # dummy_image = torch.rand((1, self.num_input_channels, *self.configuration_manager.patch_size), device=self.device)
                # dummy_text = [""]  # Or a real dummy report if needed
                # _ = self.network(dummy_image, report_text=dummy_text)
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        # print("batch type:", type(batch))  # Should say <class 'dict'>
        # print("keys:", batch.keys())       # Should show 'data', 'target', 'properties'
        # print("report_text:", batch['properties'][:1])  # Show first report text

        data = batch['data']
        target = batch['target']
        # report_text = batch['properties']['report_text']
        report_text = [prop['report_text'] for prop in batch['properties']]
        # print("[train_step] report_text:", report_text)
        # print("report_text sample (cleaned):", report_text[:1])
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        # print("[validation_step] report_text:", report_text)
    ...
    def perform_actual_validation(self, save_probabilities: bool = False, report_text=None):
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            for k in dataset_val.keys():
                ...
                try:
                    ...
                    prediction = predictor.predict_sliding_window_return_logits(data, report_text)
                except RuntimeError:
