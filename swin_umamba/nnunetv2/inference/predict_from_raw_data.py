...
class nnUNetPredictor(object):
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor, report_text: Optional[str] = None) -> torch.Tensor:
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                ...
                prediction += torch.flip(self.network(torch.flip(x, (*axes,)), report_text), (*axes,))
    ...
    def _internal_predict_sliding_window_return_logits(self,
        ...
        for sl in tqdm(slicers, disable=not self.allow_tqdm):
            ...
            prediction = self._internal_maybe_mirror_and_predict(workon, report_text)[0].to(results_device)
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor, report_text=None) \
            ...
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                if self.perform_everything_on_device and self.device != 'cpu':
                    ...
                    try:
                        ...
                        predicted_logits = self._internal_predict_sliding_window_return_logits(data, slicers, self.perform_everything_on_device, report_text)

                    except RuntimeError:
                        print('Prediction on device was unsuccessful, probably due to a lack of memory. Moving results arrays to CPU')
                        empty_cache(self.device)
                        predicted_logits = self._internal_predict_sliding_window_return_logits(data, slicers, False, report_text)
                else:
                    # predicted_logits = self._internal_predict_sliding_window_return_logits(data, slicers, self.perform_everything_on_device)
                    predicted_logits = self._internal_predict_sliding_window_return_logits(data, slicers, self.perform_everything_on_device, report_text)
