...
from nnunetv2.bert.modeling_bert import BertModel
from nnunetv2.bert.tokenization_bert import BertTokenizer
...
class SwinUMamba(nn.Module):
    def __init__(
        ...
        self.out_layers = nn.ModuleList([
            UnetOutBlock(spatial_dims, feat_size[i], out_chans) for i in range(4)
        ])
        ...
        self.lang_proj = nn.ModuleList([
            nn.Linear(768, feat_size[0]),  # for dec_out
            nn.Linear(768, feat_size[1]),  # for dec1
    ...
    def forward(self, x_in, report_text):
        ...
        for text in report_text:
            ...
            input_id = self.tokenizer.encode(text=text, add_special_tokens=True)

            token_output = self.tokenizer.encode_plus(text, padding='max_length',
                                                        max_length=24, 
                                                        truncation=True,
            ...
            # padded_input_ids[:len(input_id)] = input_id
            # attention_mask[:len(input_id)] = [1]*len(input_id)
        
            # sentences_for_ref.append(torch.tensor(padded_input_ids).unsqueeze(0))
            # attentions_for_ref.append(torch.tensor(attention_mask).unsqueeze(0))     

            sentences_for_ref.append(token)
            attentions_for_ref.append(mask)        
       
        input_ids = torch.cat(sentences_for_ref, dim=0).to(x_in.device)         # (B, T)
        attention_masks = torch.cat(attentions_for_ref, dim=0).to(x_in.device)   # (B, T)

        # BERT encoding
        l_feats = self.text_encoder(input_ids, attention_mask=attention_masks)[0]  # (B, T, 768)
        l_feats = l_feats.permute(0, 2, 1)  # (B, 768, T)
        l_mask = attention_masks.unsqueeze(-1)  # (B, T, 1)
        lang_feat = torch.mean(l_feats, dim=2)  # (B, 768)
        ...
        dec3 = dec3 + self.lang_proj[3](lang_feat).unsqueeze(-1).unsqueeze(-1).expand_as(dec3)

        dec2 = self.decoder4(dec3, enc3)
        dec2 = dec2 + self.lang_proj[2](lang_feat).unsqueeze(-1).unsqueeze(-1).expand_as(dec2)

        dec1 = self.decoder3(dec2, enc2)
        dec1 = dec1 + self.lang_proj[1](lang_feat).unsqueeze(-1).unsqueeze(-1).expand_as(dec1)

        dec0 = self.decoder2(dec1, enc1)
        dec_out = self.decoder1(dec0)
        dec_out = dec_out + self.lang_proj[0](lang_feat).unsqueeze(-1).unsqueeze(-1).expand_as(dec_out)
