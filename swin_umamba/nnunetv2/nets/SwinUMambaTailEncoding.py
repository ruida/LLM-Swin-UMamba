...
class SwinUMamba(nn.Module):
    def __init__(
        ...
        self.lang_proj = nn.ModuleList([
            nn.Linear(768, self.feat_size[0]),  # for dec_out
            nn.Linear(768, self.feat_size[1]),  # for dec1
            nn.Linear(768, self.feat_size[2]),  # for dec2
            nn.Linear(768, self.feat_size[3]),  # for dec3
        ])
    ...
    def forward(self, x_in, report_text):
        ...
        # print("len(report_text) = " + str(len(report_text)))
        # print("x_in.shape[0] = " + str(x_in.shape[0]))
        assert len(report_text) == x_in.shape[0], "Mismatch: number of reports does not match batch size of images"
        ...
        # Fuse language into decoder blocks
        feat_out = [dec_out, dec1, dec2, dec3]
        for i in range(4):
            proj = self.lang_proj[i](lang_feat)          # (B, C_i)
            proj = proj[:, :, None, None]                # (B, C_i, 1, 1)
            proj = proj.expand_as(feat_out[i])           # (B, C_i, H, W)
            feat_out[i] = feat_out[i] + proj             # Additive fusion
