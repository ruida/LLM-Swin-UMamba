...
class SwinUMamba(nn.Module):
    def __init__(
        ...
        self.text_encoder = Text_Tower(biolord_checkpoint="FremyCompany/BioLORD-2023-C")
    ...
    def forward(self, x_in, report_text):
        ...
        dec_features = []
        decoder_blocks = [
            (self.decoder6, enc_hidden, enc5),
            (self.decoder5, None, enc4),
            (self.decoder4, None, enc3),
            (self.decoder3, None, enc2),
            (self.decoder2, None, enc1),
        ]

        # First decoder layer takes enc_hidden + enc5
        x = decoder_blocks[0][0](decoder_blocks[0][1], decoder_blocks[0][2])
        x = x + self.lang_proj[4](lang_feat).unsqueeze(-1).unsqueeze(-1).expand_as(x)
        dec_features.append(x)

        # Remaining decoder layers
        for i, (decoder, _, skip) in enumerate(decoder_blocks[1:], 1):
            x = decoder(x, skip)
            x = x + self.lang_proj[4 - i](lang_feat).unsqueeze(-1).unsqueeze(-1).expand_as(x)
            dec_features.append(x)

        # Final conv block
        dec_out = self.decoder1(dec_features[-1])
        dec_out = dec_out + self.lang_proj[0](lang_feat).unsqueeze(-1).unsqueeze(-1).expand_as(dec_out)

        if self.deep_supervision:
            return [self.out_layers[i](feat) for i, feat in enumerate([dec_out] + dec_features[1:4])]
        else:
